...
Date: 2026-09-18
Starting commit and pre-existing changes: Same batch as KB-01.
Reader outcome: Local and CI validation now have matching purposes, documented commands, maintained local-link validation, maintained GitHub issue-form validation, and a versioned issue-label manifest.
Files changed: `.github/workflows/markdown-lint.yml`, `.github/workflows/link-check.yml`, `.github/workflows/terraform-format.yml`, `.github/workflows/issue-template-validation.yml`, `.github/labels.yml`, `git/github-actions/content-ci-cd-process.md`, `scripts/validate-local-links.mjs`, `scripts/test-local-link-validator.mjs`, `scripts/validate-issue-templates.py`, `scripts/test-issue-template-validator.py`, `context.md`, `CHANGELOG.md`, and this plan.
...
Publication commit or blocker: Initial validation batch published in commit `1079a0d`; issue-template validation extension published in commit `7813312`, with this record finalized in the follow-up publication-evidence commit. Label-manifest validation published in commit `cf9dcd4`; issue-template validator fixtures published in commit `76a33a5`; this record is finalized in the follow-up publication-evidence commit.
