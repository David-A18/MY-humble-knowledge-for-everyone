...
Applicable versions: Terraform 1.4.0 or newer for `terraform_data`; locally executed with Terraform v1.13.1
Validation evidence: Locally executed in `/tmp` with Terraform v1.13.1: init, validate, fmt check, plan, apply, state inspect, variable change, output, destroy, and empty state after cleanup
Known limitations: This exercise teaches Terraform workflow and local state, not cloud provider authentication, remote backends, locking, drift, or real infrastructure changes
Next review: After the first completed author run with Terraform installed
...
cp terraform/examples/local-state-lifecycle/main.tf /tmp/kb-terraform-local/
cp terraform/examples/local-state-lifecycle/.gitignore /tmp/kb-terraform-local/
...
- [Back to Terraform examples](../README.md)
